...
const excluirAutor = async (req, res) => {
  ...
  try {
    ...
    if (livros.rowCount > 0) {
      return res
        .status(401)
        .json('Autores com livros cadastrados não podem ser excluídos')
    }
    ...
    if (autorExcluido.rowCount === 0) {
      return res
        .status(404)
        .json(
          'Não foi possível excluir o autor: Autor não encontrado ou inexistente'
        )
    }

    return res.status(200).json({ messagem: 'Autor foi excluido com sucesso.' })
  } catch (error) {
    return res.status(400).json(error.message)
  }
}
